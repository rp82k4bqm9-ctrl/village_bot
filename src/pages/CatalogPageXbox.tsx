...
interface CatalogPageXboxProps {
  isAdmin?: boolean;
}
...
export function CatalogPageXbox({ isAdmin }: CatalogPageXboxProps) {
  ...
  const loadGames = async () => {
    try {
      ...
      // Фильтруем только Xbox игры
      const xboxGames = data.filter(game => 
        game.platform.some(p => p.includes('Xbox'))
      );
      setGames(xboxGames);
    } catch (err: unknown) {
  ...
  useEffect(() => {
    ...
    if (activeFilter !== 'all') {
      filtered = filtered.filter(game => {
        if (['Xbox Series X/S', 'Xbox One'].includes(activeFilter)) {
    ...
    <div className="min-h-screen bg-black p-4 pb-24">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-slate-400 text-sm">Xbox Series X/S, Xbox One игры по лучшим ценам</p>
        </div>
                  ...
                  {game.image ? (
                    <img 
                      ...
                      onError={(e) => {
                        ...
                        const fallback = document.getElementById(`fallback-xbox-${game.id}`);
